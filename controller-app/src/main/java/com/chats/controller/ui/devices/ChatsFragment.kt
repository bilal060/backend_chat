...
class ChatsFragment : Fragment() {
    ...
    companion object {
        ...
        fun newInstance(deviceId: String): ChatsFragment {
    ...
    private fun setupRealtimeUpdates() {
        deviceId?.let { id ->
            ...
            webSocketService.setDataUpdateListener { updateDeviceId, type, data ->
                ...
                if (isAdded && updateDeviceId == id && type == "chat") {
                    // Refresh chats when new data arrives
                    loadData()
                }
            }
        }
    ...
    private fun loadData() {
        ...
        deviceId?.let { id ->
            ...
            lifecycleScope.launch {
                try {
                    val response = ApiClient.getApiService().getDeviceChats(id)
                    if (response.isSuccessful && response.body()?.success == true) {
                        val chats = response.body()?.data ?: emptyList()
                        if (chats.isEmpty()) {
                            binding.textViewEmpty.text = getString(R.string.no_chats)
...
class ChatAdapter : androidx.recyclerview.widget.ListAdapter<Chat, ChatAdapter.ViewHolder>(
    ChatDiffCallback()
) {
